...
**Phase 2 的分流顺序是硬规则，不能自行跳步：**

1. 只检查**当前会话已上传/已明确提供路径/当前 workspace 已存在**的经历材料；不得为了判断有没有材料去扫描 Downloads/Desktop/Documents/Home。
2. 如果已经有足够经历材料：直接读取核验，只补岗位能力证据缺口，**不再让用户选择 GPT / Codex**。
3. 如果当前没有可用经历材料：**本轮必须只给用户两个选择并停住，不得开始经历挖掘：**
   1. GPT：把 `assets/experience-miner-prompt.md` 完整交给用户，一问一答完成后带回结果；
   2. Codex：当前会话苏格拉底式一问一答。
4. **只有用户明确回复 `2` / 选择 Codex 后**，下一轮第一问才固定为：`你之前有过工作经历吗？实习也算。`
5. 用户回复 `1` 时，交付完整 GPT Prompt 后暂停 Phase 2；用户随后改为上传现成材料时，直接读取材料，不强迫继续 1/2 分支。

禁止把“你之前有过工作经历吗？”和“如果你有旧简历也可以给我”混在同一轮作为 Phase 2 首问。那等于 Agent 自作主张选择了 Codex 路径。
