...
读取 `references/experience-input.md` 和当前 `02_<行业><岗位>核心能力.md`。

Phase 2 使用：**通用事实骨架 + 岗位能力定向深挖**。岗位能力只决定追问优先级和深度，不得暗示答案或反向制造经历。

先按当前已提供材料判断成熟度：

1. **无材料**：本轮只让用户选择 GPT / Codex，然后停止。只有用户明确选择 Codex 后，下一轮才问 `你之前有过工作经历吗？实习也算。`
2. **部分材料**：先读取，不让用户重讲；建立岗位能力证据覆盖图，只对高价值事实缺口一次问一个问题。
3. **完整事实母库**：不重跑完整访谈；先做岗位能力 × 已有经历覆盖分析。证据足够就直接通过；仍有高价值缺口才定向补问。

内部维护能力证据覆盖状态：`能力 / 重要性 / 对应经历 / 证据强度0-3 / 缺口 / 风险 / 下一问价值`。优先补 Core 能力中“很可能存在但证据没挖清”的部分，以及结果口径、ownership、判断/取舍、验收标准。

**明确没有做过的能力记为 0，不继续诱导。** 禁止问题如“你是不是用了 RAG？”、“你应该做过 A/B Test 吧？”；改问中性事实，如“当时是否需要外部知识？怎么处理？”、“你怎么比较改动前后的效果？”

无材料时的 GPT 路径：读取 `assets/experience-miner-prompt.md`，把当前目标岗位、行业、Core 能力和必要 Hard Gates/Common 填入 Prompt 的岗位定向上下文后再完整交付。

无材料时的 Codex 路径：第一问固定为 `你之前有过工作经历吗？实习也算。`，之后每轮只问一个当前信息增益最高的问题。

`03_个人经历.md` 是跨岗位复用事实母库，应增量更新，不因切换岗位整份改写成 JD 镜像。
